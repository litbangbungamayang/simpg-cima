	...
	function save() {
		...
		if( $this->form_validation->run() )
		{
			...
			if( $this->input->get( 'id_selektor' , true ) =='')
			{
            	...
            	//================== INSERT t_trash =================
				if (isset($_POST['trash_sel'])){
					$query = "insert into t_trash (id_selektor) values(?)";
...
metode_tma FROM t_spta WHERE no_spat = '".$_POST['nospta']."'")->row();
	...
	function cetak($id){
    	...
    	$query = "select * from t_trash where id_selektor =?";
		$sample_trash = $this->db->query($query, array($id))->num_rows();
		$this->data['row']->trash = "";
		if ($sample_trash == 1){
			$this->data['row']->trash = '<tr><td style="font-size: 20px;font-weight: 300">SAMPLE TRASH</td></tr>
			<tr><td>--------------------------</td></tr>';
		}
